...
- [ ] 在包含 25–60 个候选的真实案例上测候选召回率（Candidate Recall@50）和仓库重排增益。
- [ ] 增加前五成功率（Success@5）、前五归一化折损累计增益（NDCG@5）、硬约束违反率和无依据推断率。
...
- [ ] 建立可复现的冷/热缓存 p50、p95 与首个候选/首个已验证结果时间基线。
- [ ] 增加结构化追踪编号、外部调用数、Token、成本和缓存命中率。
- [ ] 流式返回逐仓库已验证结果。
- [ ] 复用多轮会话的候选、文档与证据判断，并验证缓存失效正确性。
- [ ] 评测自适应 6/8/12/24 分析预算的质量与延迟帕累托曲线。
- [ ] 实验每批 2–3 个仓库的 LLM 结构化判断。

## 优先级 3：可维护性

- [ ] 将 GitHub 搜索、文档读取和 LLM 判断的运行配置集中为类型化设置对象。
- [ ] 为节点输入输出增加更精确的类型字典，逐步减少 `dict[str, Any]`。
