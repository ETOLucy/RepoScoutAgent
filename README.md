...
当前同时提供 L1 文档证据匹配和 L2 静态实现验证；项目不执行候选仓库代码。
...
配置 `BRAVE_SEARCH_API_KEY` 后，GitHub 原生搜索与 Brave 网页搜索会并行召回。网页结果只用于发现仓库，候选仍需经过 GitHub API 补全和仓库原始证据验证。最终输出包含“需求 × 方案”证据矩阵，并将完整研究快照持久化到 `.cache/research_tasks.db`。
...
网页以多轮对话驱动同一个 Agent。用户可以在结果后继续补充或覆盖条件，服务端会用
`conversation_id` 保留最近的用户轮次，并将 Agent 的澄清问题带入下一轮。当前会话记忆仅保存在
单个服务进程内，重启后清空，不属于持久化 checkpoint。
...
- 目前以 README/docs 为主证据，Release、Issue 和最近 Commit 只作补充信息，尚未分析源码实现。
- 文档声称支持不等于功能一定正确，当前结论属于“文档证据匹配”。
- 尚未执行安装、构建和运行验证。
- 验证上限为 L2 静态实现迹象；L3 构建和 L4 运行验证明确不进入产品范围。
- Docker 部署只构建 RepoScout 自身，不会构建或启动搜索到的候选仓库。
- LLM 不可用时只能保留显式英文词，复杂中文需求需要配置模型。
- 当前读取重排后的 24 个仓库，默认对预筛选后的 8 个执行强模型判断。
- 多轮会话是有界的进程内记忆，不支持跨实例共享或重启恢复。
- 当前不使用多 Agent；仓库读取已有受控并发，拆分子 Agent 暂无已验证的质量收益。
...
停止服务使用 `docker compose down`；只有在确认不再需要文档缓存时才使用 `docker compose down -v`。
