...
网页搜索采用 provider 接口。Compose 默认使用自托管 SearXNG；配置
`BRAVE_SEARCH_API_KEY` 后 Brave 作为商业 provider 并行补充；两者都未配置时继续使用
GitHub-only。网页结果只用于发现仓库，候选仍需经过 GitHub API 补全和仓库原始证据验证。
