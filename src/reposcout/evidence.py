...
from .search.models import CorePurposeMatch, CriterionMatch
...
ASSESSMENT_PROMPT = (
    ...
    "还要判断仓库的核心用途是否与 goal 属于同一产品类别，输出 core_purpose。"
    "matched 和 mismatched 都必须引用能说明仓库核心用途的文档原文；仅仅出现需求关键词"
    "不足以判为 matched。无法判断时输出 unknown。"
...
def rule_assessment(
    ...
    goal_words = set(
        re.findall(r"[a-zA-Z][a-zA-Z0-9_.+-]{2,}", intent.goal.lower())
    )
    keyword_phrases = {
        term.casefold()
    ...
    core_term = next(
        (
            term
            for term in sorted(keyword_phrases | goal_words, key=len, reverse=True)
            if term in combined
        ),
        None,
    )
    return RepositoryAssessment(
        summary="基于文档关键词的降级匹配",
        criteria=criteria,
        core_purpose=CorePurposeMatch(
            status="matched" if core_term else "unknown",
            evidence=core_term,
            source_path=documents[0]["path"] if core_term and documents else None,
        ),
    )
...
def validate_evidence(
    ...
    core = assessment.core_purpose
    if core.status != "unknown":
        sources = [item for item in documents if item["path"] == (core.source_path or "")]
        valid_quote = bool(core.evidence) and any(
            (core.evidence or "").casefold() in item["content"].casefold()
            for item in sources
        )
        expected_shas = {item.get("commit_sha") for item in sources if item.get("commit_sha")}
        valid_sha = not expected_shas or core.source_commit_sha in expected_shas
        if not valid_quote or not valid_sha:
            assessment.core_purpose = CorePurposeMatch()
    return assessment
