...
环境中有 `GITHUB_TOKEN` 和模型密钥，但没有 `BRAVE_SEARCH_API_KEY`，因此本次只验证了
GitHub + 模型路径，不能据此声称多源召回已在该样本中生效。
...
主要劣势是总延迟仍高，文档读取和候选判断合计约 51 秒；Brave 未配置时，多源能力只是
代码能力而非本次实证；核心用途未知仍可能混入结果；当前没有冻结的同类项目集合、
